...
keywords: svelte, sveltekit, zod, errors
author: Ross Robino
date: 2023, 03, 25
---

import ZodErrorMessage from "@components/ZodErrorMessage.svelte";
...
Consider this Zod `UserSchema`, it expects a user will have an email and a password.
...
This would produce an `error` with a list of `issues`.
...
Next we can create an action for our form to validate the user's inputs on the server. If the `safeParse` is unsuccessful, we will return a list of `ZodIssue`s to render to the user.
...
export const actions = {
	default: async ({ request }) => {
		...
		if (!safeParse.success) {
			return fail(400, { error: safeParse.error.issues });
		}
...
If there's any validation issues, we want to display the messages provided by our `UserSchema`. We can create a reusable component to render these messages in a list.

```svelte
<!-- src/lib/components/ZodErrorMessage.svelte -->
...
<ul>
	{#each issues as { message, path }}
		<li>
			{String(path[0]).charAt(0).toUpperCase() +
				String(path[0]).slice(1)} -
			{message}
		</li>
	{/each}
</ul>
...
In case of an invalid form, we can now utlize our `ZodErrorMessage.svelte` component to display the issues.

```svelte
<!-- src/routes/+page.svelte -->

<script lang="ts">
	import ZodErrorMessage from "$lib/components/ZodErrorMessage.svelte";
...
<!-- if there's an error -->
{#if form?.error}
	<ZodErrorMessage issues={form.error} />
{/if}
