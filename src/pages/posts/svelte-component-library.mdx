...
Since this is a library project, it's important to add in your relevant information to the `package.json` file so it will appear in npm when the package is published. Since we are building a Svelte component library, we will add `svelte` under `"dependencies"`.
